...
- Option to display DEADLINE values as an addition (near the clocking value) on each headline
  - The default for this option is false - so there is no change to the default behavior of Organice - until you enable this option via Settings.
