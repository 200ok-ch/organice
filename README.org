...
organice implements a strategy pattern, so that adding new themes is
quite easy. These themes come bundled with organice, each in =light= and
=dark mode=:
...
organice has support for WebDAV and you can make use of that for local development.
We offer instructions for two options for setting up WebDAV servers but you
may use any other standard solution. 

- Option 1: A Docker Container with Apache
  organice ships with a Docker container with a WebDAV server based on Apache.
- Option 2: A WebDAV server in go (Docker image also available)
...
In any case, see the [[#faq_webdav][FAQ section on WebDAV]]
for details on how to get running locally with a WebDAV setup.
