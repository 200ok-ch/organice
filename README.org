...
Since organice implements Org mode, one might wonder if organice plans
to duplicate Emacs configuration strategy. In Emacs Org mode, there's
...
powerful. However, the current goal of organice is not to clone Emacs
in full. In fact, I'd argue that this is impossible. Emacs being a
LISP machine has inherent power that cannot be copied. The goal is to
make Org mode accessible for smartphones and for non-Emacs users. For
both use-cases, elisp variable configuration is not really suitable.

organice uses this customization strategy:
...
interface. Please find them in the 'settings' view (cogs item in the
header on the right).
