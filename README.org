...
(it's just a front-end application, which uses either Dropbox or
Google Drive as back-end storage). We don't store any kind of data on
our servers - we also don't use analytics on organice.200ok.ch.
...
- org-web tracks users with Google Analytics. organice [[https://github.com/200ok-ch/organice/issues/41][does not]].
