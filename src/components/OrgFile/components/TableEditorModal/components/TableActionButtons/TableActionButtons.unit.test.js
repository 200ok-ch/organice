...
describe('TableCell tests', () => {
	...
	return render(
	  <MemoryRouter keyLength={0} initialEntries={['/file/dir1/dir2/fixtureTestFile.org']}>
          <Provider store={testStore}>
            <HeaderBar />
	    <TableActionButtons filePath={testFilePath} shouldDisableActions={testShouldDisableActions}/>
          </Provider>
          </MemoryRouter>
	);
      })
  ...
  test('Test edit-cell-button', () => {
    ...
    const inTableEditMode = testStore.getState().org.present.getIn(['files', testFilePath, "editMode"]) === "table"
...
describe('Movement Tests', () => {  
  test('Test up-button', () => {
    ...
    const testCellToMoveUp = testTableContents.getIn([testRowIndex, "contents", testRandomColumnIndex])       
  ...
  test('Test left-button', () => {
    ...
    const testCellToMoveLeft = testTableContents.getIn([testRandomRowIndex, "contents", testColumnIndex])       
    ...
    const actualTable = getSelectedTable(testStore.getState())
    const actualColumnAfterMove = getContentOfTableColumn(expectedColumnIndexAfterMove, actualTable)
