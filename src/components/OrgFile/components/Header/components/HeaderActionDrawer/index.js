...
const HeaderActionDrawer = (props) => {
  ...
  const iconWithFFClickCatcher = useCallback(
    ({ className, onClick, onLongPress, title, testId = '' }) => {
  ...
  const handleDuplicateHeader = useCallback(
    onDuplicateHeader ||
      ((e) => {
    ...
    [onDuplicateHeader, onAddNewHeader]
  );
...
export default React.memo(HeaderActionDrawer);
