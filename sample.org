...
=START= and =END= can both be omitted to search for an infinite timespan into the past or future. Both can be used with digits to quantify the amount (i.e. =2w= for two weeks). For both, the digit can be omitted, so =1w= is equivalent to =w=.
...
Next to units, =START= and =END= can also be expressed in dates like.
Dates can be expressed by:
