...
*** WebDAV test server, option 2

An alternative option is downloading either the binary for your platform or 
a docker image from 
[[https://github.com/hacdias/webdav/][webdav]] project and creating a configuration file
following an example displayed in the instructions.
This is a WebDAV implemented in go and has binaries for many platforms. It can 
be even run on Android in Termux, so you can have a full offline and mobile solution.

Here is the minimum configuration that will get you going. Note that it does not use
authentication. If you want a more involved configuration, consult the documentation 
linked above.
...
Save this as =config.yaml= and then simply run =webdav= from the same directory.
Otherwise, you may specify the config file with =-c path-to-config.yaml= parameter.
